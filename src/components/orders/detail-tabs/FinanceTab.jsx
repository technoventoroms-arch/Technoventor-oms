...
export function FinanceTab({ order, activeTab, financeData, setEditingFinanceItemId, editingFinanceItemId, isSubmittingFinance, saveFinance, updateFinanceItem, hasPermission, PERMISSIONS }) {
  ...
  if (purchases.length === 0) {
    return (
      <div className="text-center py-20 bg-white dark:bg-slate-900 border border-dashed border-border rounded-3xl">
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-text-muted text-xs font-medium uppercase tracking-widest mt-0.5">Approve POs one by one — No. of vendors = No. of POs</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        {Array.from(poGroups.entries()).map(([poKey, poGroup], idx) => {
          ...
          return (
            <div key={poKey} className={`bg-white dark:bg-slate-900 border transition-all duration-300 rounded-2xl overflow-hidden ${isEditing ? 'border-emerald-500/50 shadow-2xl shadow-emerald-500/10 scale-[1.01]' : 'border-border'}`}>
              <div className="bg-surface-raised p-5 border-b border-border flex justify-between items-center bg-gradient-to-r from-slate-800/20 to-transparent">
                <div className="flex gap-4">
                  <div className="w-12 h-12 bg-white dark:bg-slate-950/50 rounded-xl flex items-center justify-center border border-border text-text-secondary font-mono text-xs font-bold">
                    PO{idx + 1}
                  </div>
                  <div>
                    <p className="text-sm font-bold text-text-primary mb-1">{poGroup.vendorName}</p>
                    <div className="flex items-center gap-2">
                     <span className="text-[10px] bg-surface-raised dark:bg-slate-800 text-emerald-700 dark:text-emerald-400 px-2 py-0.5 rounded font-bold uppercase tracking-wider">{poGroup.poNumber || 'PO PENDING'}</span>
                     <span className="text-[10px] text-text-muted font-bold">{poGroup.items.length} item{poGroup.items.length !== 1 ? 's' : ''}</span>
                     {poGroup.poValue && <span className="text-[10px] text-emerald-700 dark:text-emerald-400/70 font-mono">₹{Number(poGroup.poValue).toLocaleString('en-IN')}</span>}
                    </div>
                  </div>
                </div>
                ...
                {!isEditing && hasPermission(PERMISSIONS.EDIT_FINANCE) && (
                  <button
                    ...
                    className="group flex items-center gap-2 px-4 py-2 bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 rounded-xl text-xs font-black uppercase tracking-widest border border-emerald-500/20 hover:bg-emerald-500 hover:text-text-primary transition-all shadow-lg hover:shadow-emerald-500/20"
                  >
              ...
              <div className="p-4 border-b border-border/40 bg-white dark:bg-slate-950/10">
              ...
              {isEditing ? (
                <div className="p-6 bg-slate-50 dark:bg-slate-950/20 animate-in fade-in slide-in-from-top-4 duration-500">
                  <div className="grid grid-cols-2 gap-6">
                    <FormField label="PO Value (Reference)">
                      <div className="w-full bg-white dark:bg-slate-900 border border-border rounded-xl px-4 py-3 text-emerald-700 dark:text-emerald-400 font-mono text-sm font-bold">
                    ...
                    {hasPermission(PERMISSIONS.APPROVE_PAYMENTS) && (
                      <FormField label="Payment Status">
                        <select
                          ...
                          className="w-full bg-white dark:bg-slate-900 border border-border rounded-xl px-4 py-3 text-text-primary focus:ring-2 focus:ring-emerald-500/40"
                        >
                    ...
                    <FormField label="Approval Date">
                      <input
                        ...
                        className="w-full bg-white dark:bg-slate-900 border border-border rounded-xl px-4 py-3 text-text-primary"
                      />
                    </FormField>
                    <FormField label="Payment Reference">
                      <input
                        ...
                        className="w-full bg-white dark:bg-slate-900 border border-border rounded-xl px-4 py-3 text-text-primary font-mono"
                  ...
                  <div className="flex justify-end gap-3 mt-6">
                    <button onClick={() => setEditingFinanceItemId(null)} disabled={isSubmittingFinance} className="px-6 py-2.5 text-text-muted font-bold hover:text-text-primary transition-colors uppercase text-[10px] tracking-widest disabled:opacity-50">Cancel</button>
                    <button onClick={saveFinance} disabled={isSubmittingFinance} className="flex items-center gap-2 px-10 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-xl font-black uppercase text-[10px] tracking-widest shadow-xl shadow-emerald-500/20 disabled:opacity-50">
                      {isSubmittingFinance && <span className="w-3 h-3 btn-spinner"></span>}
                ...
                <div className="p-5">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div className="flex flex-col gap-1 p-3 bg-slate-50 dark:bg-slate-950/20 rounded-xl border border-border/40">
                      <span className="text-[10px] font-bold text-text-muted uppercase">Payment Status</span>
                      <div className="flex items-center gap-2 pt-1">
                        ...
                        <span className={`text-xs font-black uppercase ${(itemFinance.paymentStatus === 'Completed' || itemFinance.paymentStatus === 'Partial Payment') ? 'text-emerald-700 dark:text-emerald-400' : 'text-text-muted'}`}>
