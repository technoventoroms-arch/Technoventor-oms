...
const STATUS_COLORS = {
  ...
  'Not Applicable': 'bg-slate-500/10 text-text-secondary border-slate-500/20',
};
...
export function OrderPlanningTab({ order, onUpdate, currentUser, canEdit = true }) {
  ...
  const inputCls = 'w-full bg-surface-raised dark:bg-slate-800 border border-border rounded-xl px-3 py-2 text-sm text-text-primary placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-emerald-500/40 transition';
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-text-muted text-xs mt-1">Plan delivery, BOQ execution, and stage timelines</p>
        </div>
        {canEdit && (
          <button
            onClick={handleSave}
      ...
      {/* Project Info */}
      <div className="bg-slate-50 dark:bg-slate-900/60 border border-border rounded-2xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label className="block text-xs font-semibold text-text-muted mb-1.5">Delivery Location</label>
            <input
              className={inputCls}
          ...
          <div>
            <label className="block text-xs font-semibold text-text-muted mb-1.5">Project Start Date</label>
            <input
              type="date"
              className={inputCls}
          ...
          <div>
            <label className="block text-xs font-semibold text-text-muted mb-1.5">Project End Date</label>
            <input
              type="date"
              className={inputCls}
      ...
      <div className="bg-slate-50 dark:bg-slate-900/60 border border-border rounded-2xl overflow-hidden">
        <div 
          className="px-6 py-4 border-b border-border flex justify-between items-center cursor-pointer hover:bg-surface-raised transition-colors"
        ...
        >
          <h4 className="text-sm font-bold text-text-secondary uppercase tracking-widest flex items-center gap-2">
            <Package className="w-4 h-4 text-emerald-700 dark:text-emerald-400" /> BOQ Item-wise Delivery Plan
          </h4>
          {isBoqExpanded ? <ChevronUp className="w-4 h-4 text-text-muted" /> : <ChevronDown className="w-4 h-4 text-text-muted" />}
        </div>
        {isBoqExpanded && (
          <div className="overflow-x-auto animate-in fade-in duration-300">
            ...
            <thead className="bg-surface-raised">
              <tr>
                <th className="text-left p-4 text-xs font-semibold text-text-muted uppercase">#</th>
                <th className="text-left p-4 text-xs font-semibold text-text-muted uppercase">Item</th>
                <th className="text-left p-4 text-xs font-semibold text-text-muted uppercase">Total Qty</th>
                <th className="text-left p-4 text-xs font-semibold text-text-muted uppercase">Planned Qty</th>
                <th className="text-left p-4 text-xs font-semibold text-text-muted uppercase">Delivery Date</th>
                <th className="text-left p-4 text-xs font-semibold text-text-muted uppercase">Status</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {planning.boqItems.map((item, idx) => (
                <tr key={item.itemId} className="hover:bg-surface-raised/20 transition-colors">
                  <td className="p-4 text-text-muted font-mono text-sm">{idx + 1}</td>
                  <td className="p-4">
                    <span className="text-sm font-medium text-text-primary">{item.itemName}</span>
                    <span className="block text-xs text-text-muted mt-0.5">{item.unit}</span>
                  </td>
                  <td className="p-4 text-text-secondary font-mono text-sm">{item.totalQty}</td>
                  <td className="p-4">
                    <input
                      type="number"
                      className="w-24 bg-surface-raised dark:bg-slate-800 border border-border rounded-lg px-3 py-1.5 text-sm text-text-primary focus:outline-none focus:ring-1 focus:ring-emerald-500/40"
                  ...
                  <td className="p-4">
                    <input
                      type="date"
                      className="bg-surface-raised dark:bg-slate-800 border border-border rounded-lg px-3 py-1.5 text-sm text-text-primary focus:outline-none focus:ring-1 focus:ring-emerald-500/40"
            ...
            <div key={idx} className="p-4">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-semibold text-text-primary text-sm">{stage.stage}</span>
      ...
      <div className="bg-slate-50 dark:bg-slate-900/60 border border-border rounded-2xl overflow-hidden">
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          {planning.milestones.map((m, idx) => (
            <div key={idx} className="flex items-center gap-3 p-4 bg-surface-raised border border-border rounded-xl">
              <div className="w-2 h-2 rounded-full bg-amber-400 flex-shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-text-primary truncate">{m.title}</p>
                <input
                  type="date"
                  className="mt-1.5 w-full bg-surface-raised dark:bg-slate-800 border border-border rounded-lg px-3 py-1.5 text-xs text-text-secondary focus:outline-none focus:ring-1 focus:ring-amber-500/40"
                  value={m.date}
