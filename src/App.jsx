...
import { formatDateTime, hasRaisedProcurementPO } from './utils';
import * as api from './api';
import { usePermissions } from './hooks/usePermissions';
import { useOrders } from './hooks/useOrders';
import { LoginScreen } from './components/auth/LoginScreen';
import { NotificationPanel, AccessDenied, Toast, ThemeToggle } from './components/common';
...
function App() {
  ...
  if (isAuthLoading) return <div className="min-h-screen bg-page flex items-center justify-center"><div className="w-12 h-12 border-4 border-emerald-500 border-t-transparent rounded-full animate-spin"></div></div>;
  if (!currentUser) return <LoginScreen onLogin={handleLogin} />;

  return (
    <div className="min-h-screen bg-page text-text-primary font-sans selection:bg-emerald-500/30">
      <div className="flex h-screen overflow-hidden">
        ...
        <main className="flex-1 flex flex-col relative overflow-hidden bg-page dark:bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] dark:from-slate-900 dark:via-[#020617] dark:to-[#020617]">
          <header className="h-16 border-b border-border flex items-center justify-between px-8 bg-white/80 dark:bg-slate-900/30 backdrop-blur-md sticky top-0 z-10">
            <div className="flex items-center gap-4">
              <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 hover:bg-surface-raised rounded-lg text-text-secondary"><Menu className="w-5 h-5" /></button>
              <h1 className="text-lg font-bold text-text-primary uppercase tracking-widest">{MODULE_LABELS[view] || 'Overview'}</h1>
            </div>
            <div className="flex items-center gap-6">
              <ThemeToggle />
              <div className="relative">
                <button onClick={() => setShowNotifications(!showNotifications)} className="p-2 hover:bg-surface-raised rounded-lg text-text-secondary relative"><Bell className="w-5 h-5" /><span className="absolute top-2 right-2 w-4 h-4 bg-emerald-500 rounded-full border-2 border-page text-[10px] flex items-center justify-center font-bold text-white">{notifications.length}</span></button>
                {showNotifications && <NotificationPanel notifications={notifications} onClose={() => setShowNotifications(false)} />}
              </div>
              <div className="flex items-center gap-3 px-4 py-1.5 bg-surface-raised border border-border rounded-xl">
                ...
                <div className="hidden md:block"><p className="text-sm font-bold text-text-primary leading-none">{currentUser?.name}</p><p className="text-[10px] text-text-muted font-medium uppercase mt-0.5">{currentUser?.department}</p></div>
              </div>
            </div>
          </header>
